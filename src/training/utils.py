...
def build_vfm(name, weight=None):
    sam_ckpts = {
    ...
    dinov3_ckpts = {
        "dinov3-L": "dinov3_vitl16",
        "dinov3-B": "dinov3_vitb16",
    }
    ...
    # DINOv3
        ...
        if name in dinov3_ckpts:
            model_name = dinov3_ckpts[name]
            try:
                vfm = torch.hub.load(
                    './src/dinov3',
                    model_name,
                ...
                raise RuntimeError(f"Failed to load DINOv3 model '{name}': {e}")
        else:
            raise NotImplementedError(f"VLM model '{name}' not supported under DINOv3 category.")
