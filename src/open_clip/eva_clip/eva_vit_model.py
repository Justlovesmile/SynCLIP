...
class Block(nn.Module):
    ...
    def forward_without_rcffn(self, x, mode):
        if self.gamma_1 is None:
            if self.postnorm:
                x = self.drop_path(self.norm1(self.attn.ss_attn(x, mode)))
            else:   
                x = self.drop_path(self.attn.ss_attn(self.norm1(x),mode))
        else:
            if self.postnorm:
                x = self.drop_path(self.gamma_1 * self.norm1(self.attn.ss_attn(x, mode)))
            else:
                x = self.drop_path(self.gamma_1 * self.attn.ss_attn(self.norm1(x), mode))
        return x
...
class EVAVisionTransformer(nn.Module):
    ...
    def encode_dense(self, x, keep_shape=True, mode="qq"):
        bs, _, h, w = x.shape
        h = h // self.patch_embed.patch_size[0]
        w = w // self.patch_embed.patch_size[1]
        x = self.patch_embed(x)
        ...
        for blk in self.blocks[:-1]:
            x = blk(x, rel_pos_bias=rel_pos_bias)
        if "distill" in mode:
            x, extra_feats = self.blocks[-1].forward_without_rcffn(x, mode)
        else:
            if mode == "maskclip":
                x = self.blocks[-1].forward_without_attn(x)
            elif mode == "vanilla" or mode == "sanity_check":
        ...
        if "distill" in mode:
            return x, extra_feats
        elif mode == "sanity_check":
            return x, x
        else:
            return x

    def extract_roi_features(self, x, normed_boxes, mode="qq"):
        if 'distill' in mode or mode=="sanity_check":
            x, extra_feats = self.encode_dense(x, keep_shape=True, mode=mode)
            return roi_align(x, self._denormalize_boxes(normed_boxes, x), (1, 1), 1.0, -1, True)[..., 0, 0], extra_feats
        else:
            x = self.encode_dense(x, keep_shape=True, mode=mode)
    ...
    def mask_pool(self, x, masks,mode="qq"):
        feature_map = self.encode_dense(x, keep_shape=False,mode=mode)
        num_masks_per_image = [len(masks_per_image) for masks_per_image in masks]
        masks = torch.cat(masks).float().flatten(-2, -1)    # bs, h*w
        feature_map = torch.repeat_interleave(feature_map, torch.tensor(num_masks_per_image, device=feature_map.device), dim=0)
