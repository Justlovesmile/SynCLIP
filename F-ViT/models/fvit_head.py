...
class FViTBBoxHead(ConvFCBBoxHead):
    ...
    def forward(self, x, vlm_box_feats=None, text_embed=None):
        if text_embed is None:
            all_embed = self.all_embed.type_as(x)
        else:
            all_embed = [text_embed[name] for name in self.all_classes]
            all_embed = torch.stack(all_embed, dim=0).permute(1, 0).contiguous()
            all_embed = F.normalize(all_embed, p=2, dim=0)
            all_embed = all_embed.type_as(x)
...
class FViTRoIHead(StandardRoIHead):
    ...
    def simple_test_bboxes(self,
        ...
        bbox_results = self._bbox_forward(x, rois, vlm_feat=vlm_feat, text_embed=text_embed)
    ...
    def _bbox_forward(self, x, rois, vlm_feat=None, text_embed=None):
        ...
        if text_embed is None:
            ...
            cls_score, bbox_pred = self.bbox_head.inference(bbox_feats, vlm_roi_feats, text_embed)
