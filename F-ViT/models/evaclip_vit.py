...
class EvaCLIPViT(BaseModule):
    ...
    def init_weights(self):
        ...
        # frozen from DeCLIP CAT-Seg branch
        if 'eva' in self.model_name.lower():
            for name, params in self.visual.named_parameters():
                if 'blocks' in name:
                    if "attn" in name:
    ...
    def forward(self, x):
        ...
        # [code from DeCLIP-CAT-Seg]
        x = visual.blocks[-1].forward_without_rcffn(x, mode='csa')
